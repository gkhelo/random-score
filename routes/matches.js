...
router.get('/filter/:day', (req, res) => {
    matchController.getByDay(req.params.day).then(matches => {
        res.send(JSON.stringify(utils.groupByLeague(matches)));
    });
});

router.get('/filter/:day/:statuses', (req, res) => {
    let statuses = JSON.parse(req.params.statuses);
    
    matchController.getByDayAndStatuses(req.params.day, statuses).then(matches => {
        res.send(JSON.stringify(matches));
    });
});

router.get('/live', (req, res) => {
    matchController.getLiveMatches().then(matches => {
        res.send(JSON.stringify(matches));
    });
});
